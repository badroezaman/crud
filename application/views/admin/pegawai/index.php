...
<a href="<?php echo site_url('Pegawai/add_datadiri'); ?>" class="btn btn-success btn-sm"><i class="icon-user-plus"></i> Tambah</a>
<?= br(2); ?>
<?php
            ...
            <tbody>
                <?php foreach ($all as $row) :; ?>
                    <tr>
                        ...
                        <td><?= $row->no_telp ?></td>
                        <td><?= $row->j_kelamin ?></td>
                        <td><?php $agm = $this->db->where('id_agama', $row->id_agama)->get('agama')->row_array();
                                echo $agm['agama'] ?></td>
                        <td><a href="<?php echo base_url() . 'upload/pegawai/ijazah/' . $row->ijazah; ?>">DOWNLOAD</a></td>
                        <td class="text-center">
                            <a href="<?php echo site_url('Pegawai/edit_datadiri/' . $row->id_pegawai); ?>" class="btn btn-info btn-xs" data-popup="tooltip" data-original-title="Edit Data" data-placement="top"><i class="icon-pencil5"></i></a>
